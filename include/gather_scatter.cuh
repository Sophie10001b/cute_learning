...
__global__ void gather_scatter_kernel(const __grid_constant__ GatherScatterParams params) {
    ...
    const uint32_t warp_row_offset = bidx * kRowPerCTA + warp_id % kRowPerCTA;
    ...
    // 4. mainloop
    constexpr uint32_t k_range = shape<1,1>(gA_row);
    #pragma unroll
    for (uint32_t i=0; i < k_range; ++i) {
        auto gA = gA_row(make_coord(_, _), make_coord(mA_idx, i));
        auto gB = gB_row(make_coord(_, _), make_coord(mB_idx, i));
